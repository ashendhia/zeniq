...
const Services = () => {
    ...
    const zen = [
        "ZENIQ Coin est activée sans pré-minage",
        "Émis par l'exploitation minière (1 %) et le monnayage (99 %) sur le ZENIQ Hub uniquement",
        "Services de paiement cryptés POS.",
        "À utiliser pour la tokenisation où les propriétaires peuvent participer à la propriété partielle d'actifs physiques tels que l'immobilier, l'art, les métaux précieux, etc."
    ]

    const pay = [
        "Technologie cryptographique pour le commerce électronique.",
        "API pour les guichets automatiques Crypto.",
        "Services de paiement cryptés POS.",
        "API du fournisseur de services de paiement (similaire à Paypal/Bitpay)."
    ]

    const nomo = [
        "Convivial : suivez et échangez facilement des crypto-monnaies en temps réel dans le portefeuille.",
        "Décentralisé : application multi-actifs non dépositaire sans exigences KYC.",
        "Paiement POS avec Crypto et fiat.",
        "Le propre portefeuille sécurisé de l'utilisateur sur le smartphone.",
        "Sécurisé : connecté à ZENIQ Hub, qui protège les actifs numériques de l'utilisateur grâce à son composant de sécurité intégré."
    ]
    ...
    return (
            ...
            <header ref={hero} class="smSv md:mdSv ">
                <div className={` top-[34.05797101449276vw] mx-auto relative col items-center text-center md:text-start md:items-start gap-[0.5256vw] md:absolute md:top-[14.25755584756899vw] md:left-[47.89750328515112vw]`} >
                    <h3 class={`smallTitle hiddenDown ${heroVisible ? 'showY' : ''} `} >
                        Écosystème au-delà de la
                    </h3>
                    <h1 class={`bigTitle hiddenDown ${heroVisible ? 'showY delay-200' : ''}`}>
                        Realité
                    </h1>
                    <p class={`heroDescription hiddenDown ${heroVisible ? 'showY delay-300' : ''}`}>
                        Découvrez les différents services que ZENIQ SAFIR met à la disposition de ses utilisateurs et clients.
                    </p>
                </div>
                <Image class={` mt-[40vw] mx-auto relative md:mt-0 md:absolute md:top-[7.1616294349540075vw] md:left-[17.082785808147175vw] servicesPic `} src={services} alt="services" />
            ...
            <div class="services" >
                <section ref={zenCoin} id="Zeniq Coin" class={`col items-center md:items-start flex-col-reverse md:section `}>
                    ...
                    <div class={`col ${viewPort ? 'hiddenDown' : ''} md:hiddenRight ${zenCoinVisible && viewPort ? 'showY' : ''} ${zenCoinVisible ? 'md:showX' : ''} `}>
                        ...
                        <p class="boldParagraph mt-[7.246376811594203vw] md:mt-[2.3653088042049935vw]">
                            ZENIQ Coin est le jeton natif et la pierre angulaire de l'écosystème financier créé avec la blockchain ZENIQ.
                        </p>
                ...
                <section ref={zenPay} id="Zeniq Pay" class="col items-center md:section">
                    <div class={`col ${viewPort ? 'hiddenDown' : ''} md:hiddenRight ${zenPayVisible && viewPort ? 'showY' : ''} ${zenPayVisible ? 'md:showX' : ''} `}>
                        ...
                        <p class="paragraph text-start mt-[10.38647342995169vw] md:mt-[2.3653088042049935vw]">
                            Avec notre écosystème blockchain unique et nos partenaires pour intégrer les paiements fiat, nous développons des solutions qui simplifient et rationalisent les paiements en ligne et POS. ZENIQ Pay est une solution de technologie cryptographique tout-en-un pour votre projet, qu'il s'agisse d'une place de marché, d'un portefeuille, d'un service de paiement, d'un service de guichet automatique ou d'une boutique en ligne. Intégrez ZENIQ Pay pour booster votre projet avec une solution fiable et polyvalente.
                        </p>
                ...
                <section ref={zenExc} id="Zeniq Exchange" class="col flex-col-reverse items-center md:section">
                    ...
                    <div class={`col ${viewPort ? 'hiddenDown' : ''} md:hiddenRight ${zenExcVisible && viewPort ? 'showY' : ''} ${zenExcVisible ? 'md:showX' : ''} `}>
                        ...
                        <p class="paragraph text-start mt-[10.38647342995169vw] md:mt-[2.3653088042049935vw]">
                            En tant qu'échange décentralisé, ZENIQ Exchange se concentre principalement sur la sécurité, la confidentialité et un matériel facile à utiliser pour les clients. ZENIQ permet à sa communauté d'échanger sur tous les actifs cryptographiques en toute sécurité en toute simplicité. ZENIQ offre un moyen rapide et sécurisé de gérer et d'échanger toutes les catégories d'actifs cryptographiques en utilisant les avantages d'un échange décentralisé. D'autre part, ZENIQ élimine les inconvénients des solutions centralisées actuelles sur le marché en utilisant sa propre blockchain décentralisée.
                        </p>
                    </div>
                </section>
                <section ref={nomoApp} id="Nomo" class="col items-center md:section">
                    <div class={`col ${viewPort ? 'hiddenDown' : ''} md:hiddenRight ${nomoAppVisible && viewPort ? 'showY' : ''} ${nomoAppVisible ? 'md:showX' : ''} `}>
                        ...
                        <p class="boldParagraph text-start w-full md:w-[43.823915900131404vw] mt-[5.314009661835748vw] md:mt-[1.8396846254927726vw]">
                            Le portefeuille blockchain préféré et approuvé par plus de 100 000 utilisateurs.
                        </p>
                        <p class="paragraph text-start w-full md:w-[43.823915900131404vw] mt-[4.3478260869565215vw] md:mt-[1.5768725361366622vw]">
                            Nomo propulsé par ZENIQ est un portefeuille polyvalent et multi-actifs qui est équipé de fonctionnalités telles que les swaps intégrés, le stockage NFT, les transactions cryptographiques en temps réel, entre autres. L'application est connectée au ZENIQ HUB et chaque transaction est déclenchée dans l'application ZENIQ et implémentée dans le ZENIQ HUB.
                        </p>
        ...
        /*<main>
                        ...
                        <p class="paragraph w-[43.823915900131404vw] mt-[1.5768725361366622vw]">
                            Nomo propulsé par ZENIQ est un portefeuille polyvalent et multi-actifs qui est équipé de fonctionnalités telles que les swaps intégrés, le stockage NFT, les transactions cryptographiques en temps réel, entre autres. L'application est connectée au ZENIQ HUB et chaque transaction est déclenchée dans l'application ZENIQ et implémentée dans le ZENIQ HUB.                        </p>
                        <ul class="ul">
                            {nomo.map((key, i) => {
                                return (
                                    <li key={i} class="li">
                        ...
                        <div class="row mt-[3.679369250985545vw] gap-[1.1826544021024967vw]">
                            <Image class="appStore" src={appstore} />
                            <a href="https://play.google.com/store/apps/details?id=com.zeniq.app" target="_blank">
                                <svg class="playStore" viewBox="0 0 217.62 64.05">
                                    <defs>
                                        <linearGradient id="linear-gradient" x1="0.808" y1="0.5" x2="-1.573" y2="0.5" gradientUnits="objectBoundingBox">
                                            <stop offset="0" stop-color="#f7df14" />
                    ...
                    <div class="md:mt-[6.57030223390276vw] md:mr-[10.3311432325887vw]">
                        <Image class="app" src={app} alt="nomo" />
                    </div>
                </section>
            </div>
        </main>*/
