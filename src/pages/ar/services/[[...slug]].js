...
const Services = () => {
    ...
    const zen = [
        "عملة ZENIQ مفعلة بدون تعدين مسبق.",
        "يتم إصدارها من خلال التعدين (1٪) والصك (99٪) في ZENIQ Hub فقط.",
        "خدمات الدفع المشفرة في نقاط البيع.",
        "تستخدم في الترميز حيث يمكن لمالكي الرموز المشاركة في الملكية الجزئية للأصول المادية مثل العقارات والفنون والمعادن الثمينة ، إلخ."
    ]

    const pay = [
        "تقنية التشفير للتجارة الإلكترونية.",
        "API لأجهزة الصرف الآلي المشفرة.",
        "خدمات الدفع المشفرة في نقاط البيع.",
        "مزود خدمة الدفع API (على غرار Paypal / Bitpay)."
    ]

    const nomo = [
        "سهل الاستخدام: تتبع العملات المشفرة وتداولها في الوقت الفعلي داخل المحفظة بسهولة.",
        "لا مركزي: تطبيق متعدد الأصول غير أمني بدون متطلبات اعرف عميلك.",
        "خدمات الدفع المشفرة و الورقية في نقاط البيع.",
        "المحفظة الآمنة الخاصة بالمستخدم على الهاتف الذكي.",
        "آمن: متصل بـ ZENIQ Hub، الذي يحمي الأصول الرقمية للمستخدم من خلال مكون الأمان المدمج."
    ]

    return (
        <main>
            <header ref={hero} class="smSv md:mdSv ">
                <div dir="rtl" className={` top-[34.05797101449276vw] mx-auto relative col items-center text-center md:text-start md:items-start gap-[0.5256vw] md:absolute md:top-[14.25755584756899vw] md:left-[47.89750328515112vw]`} >
                    <h3 class={`smallTitleAR hiddenDown ${heroVisible ? 'showY' : ''} `} >
                        نظام معلوماتي يفوق
                    </h3>
                    <h1 class={`bigTitleAR hiddenDown ${heroVisible ? 'showY delay-200' : ''}`}>
                        الواقع
                    </h1>
                    <p class={`heroDescriptionAR md:mt-[1.5768725361366622vw] hiddenDown ${heroVisible ? 'showY delay-300' : ''}`}>
                        اكتشف الخدمات المختلفة التي تقدمها شركة سفير وزينيك لمستخدميها وعملائها.
                    </p>
                </div>
            ...
            <div class="services" >
                <section ref={zenCoin} id="Zeniq Coin" class={`col items-center md:items-start flex-col-reverse md:section `}>
                    ...
                    <div dir="rtl" class={`col ${viewPort ? 'hiddenDown' : ''} md:hiddenRight ${zenCoinVisible && viewPort ? 'showY' : ''} ${zenCoinVisible ? 'md:showX' : ''} `}>
                        <h2 class="titleAR">
                            عملة زينيك
                        </h2>
                        <p class="boldParagraph mt-[7.246376811594203vw] md:mt-[2.3653088042049935vw]">
                            عملة ZENIQ هي الرمز الأصلي وشريان الحياة للنظام المعلوماتي المالي الذي تم إنشاؤه باستخدام سلسلة ZENIQ blockchain.
                        </p>
                ...
                <section ref={zenPay} id="Zeniq Pay" class="col items-center md:section">
                    <div dir="rtl" class={`col ${viewPort ? 'hiddenDown' : ''} md:hiddenRight ${zenPayVisible && viewPort ? 'showY' : ''} ${zenPayVisible ? 'md:showX' : ''} `}>
                        <h2 class="titleAR">
                            ZENIQ Pay
                        </h2>
                        <p class="paragraph text-start mt-[10.38647342995169vw] md:mt-[2.3653088042049935vw]">
                            من خلال نظامنا المعلوماتي الفريد المعتمد عل زينيك blockchain وشركائنا لدمج المدفوعات الورقية، نقوم بتطوير الحلول التي تبسط وتيسر المدفوعات عبر الإنترنت ونقاط البيع. ZENIQ Pay هو حل تكنولوجي لتشفير شامل لمشروعك، سواء كان سوقًا ،محفظة ،خدمة دفع أو خدمة صراف آلي، أو متجرًا عبر الإنترنت. ادمج ZENIQ Pay لتعزيز مشروعك من خلال حل موثوق متعدد الاستخدامات.
                        </p>
                ...
                <section ref={zenExc} id="Zeniq Exchange" class="col flex-col-reverse items-center md:section">
                    ...
                    <div dir="rtl" class={`col ${viewPort ? 'hiddenDown' : ''} md:hiddenRight ${zenExcVisible && viewPort ? 'showY' : ''} ${zenExcVisible ? 'md:showX' : ''} `}>
                        <h2 class="titleAR">
                            ZENIQ Exchange
                        </h2>
                        <p class="paragraph text-start mt-[10.38647342995169vw] md:mt-[2.3653088042049935vw]">
                            بصفتها بورصة لامركزية، ينصب التركيز الرئيسي لـ ZENIQ Exchange على الأمان والخصوصية وسهولة استخدام الأجهزة للعملاء. تعمل ZENIQ على تمكين مجتمعهم من التداول عبر أي أصول تشفير بأمان وبساطة. تقدم ZENIQ طريقة سريعة وآمنة لإدارة وتداول جميع فئات أصول التشفير باستخدام مزايا التبادل اللامركزي. من ناحية أخرى ، تعمل زينيك على التخلص من عيوب الحلول المركزية الحالية في السوق باستخدام blockchain اللامركزي الخاص بها.
                        </p>
                    </div>
                </section>
                <section ref={nomoApp} id="Nomo" class="col items-center md:section">
                    <div dir="rtl" class={`col ${viewPort ? 'hiddenDown' : ''} md:hiddenRight ${nomoAppVisible && viewPort ? 'showY' : ''} ${nomoAppVisible ? 'md:showX' : ''} `}>
                        <div class="row items-start gap-[1.2077294685990339vw] md:gap-[0.5256241787122208vw]">
                            <p class="mt-[4.272946859903382vw] md:mt-[2.956636005256242vw] font-messiri font-medium text-newBlack text-[2.1739130434782608vw] md:text-[1.1826544021024967vw] ">
                                powered by ZENIQ
                            </p>
                            <h2 class="titleAR">
                                Nomo
                        ...
                        <p class="boldParagraph text-start w-full md:w-[43.823915900131404vw] mt-[5.314009661835748vw] md:mt-[1.8396846254927726vw]">
                            محفظة blockchain و الموثوقة من قبل أكثر من 100000 مستخدم.
                        </p>
                        <p class="paragraph text-start w-full md:w-[43.823915900131404vw] mt-[4.3478260869565215vw] md:mt-[1.5768725361366622vw]">
                            Nomo المدعوم من زينيك، عبارة عن محفظة متعددة الاستخدامات ومتعددة الأصول، مجهزة بميزات مثل المقايضات المضمنة وتخزين NFT ومعاملات التشفير في الوقت الفعلي، من بين أمور أخرى. التطبيق متصل بـ ZENIQ HUB ويتم تشغيل كل معاملة في تطبيق زينيك ويتم تنفيذها في ZENIQ HUB.
                        </p>
